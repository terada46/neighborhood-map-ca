...
function initApp() {
	...
	var ViewModel = function() {
		...
		//当点击列表项时，仅传递id去执行openWindow
		self.updateList = function(id) {
			self.openWindow(id, null);
		};
		...
		self.openWindow = function(mark_id, marker) {
			...
			requestApi(selectedMarker, view.map);
		}


		//marker的构造函数，除了创建marker的option，添加进markers数组，并为其绑上点击事件
		self.createMarkers = function(location) {
			var markersOption = {
...
//同时使用flickr和wikipedia的API，获取非Google的第三方数据
function requestApi(selectedMarker, map) {
	...
	.then(function(result1, result2) { 
		...
		self.InfoMarker.open(map, selectedMarker);
...
//处理加载Google Map出错提示
function mapError() {
	alert('Google Map initialized Error! Please check if there is a network error and try again.');
};
